<?php
    ...
    {
        if (isset($_GET["adapter_id"]) and isset($_GET["adapter_beacon"]))
        {
            $result = @$link->query('UPDATE `adapters` SET `adapter_beacon` = '.$_GET["adapter_beacon"].' WHERE `adapters`.`adapter_id` = '.$_GET["adapter_id"]);
