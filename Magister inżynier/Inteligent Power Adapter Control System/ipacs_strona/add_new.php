<?php
    ...
    else
    {
        ...
        {
            $result = @$link->query('INSERT INTO `sockets` (`socket_id`, `adapter_id`, `socket_name`, `socket_state`, `socket_task_control`) VALUES (NULL, "'.$_GET["adapter_id"].'", "'.$_POST["socketName"].'", "0", "0")');
            //echo 'INSERT INTO `sockets` (`socket_id`, `adapter_id`, `socket_name`, `socket_state`, `socket_task_control`, `socket_task_assigned`) VALUES (NULL, "'.$_GET["adapter_id"].'", "'.$_POST["socketName"].'", "0", "0", "0")';
        ...
        else if (isset($_POST["addButton"]) and isset($_GET["adapter_id"]) and isset($_GET["adapter"]) and isset($_POST["adapterRoom"]) and isset($_POST["adapterLocation"]))
        {
            ...
            $result->free();
            $result = @$link->query('INSERT INTO `stats` (`adapter_id`, `packets_recieved`, `packets_sent`) VALUES ("'.$row['adapter_id'].'", "1", "1")');
