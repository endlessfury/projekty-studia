...
void setup() {
  ...
  radio.begin();
  radio.openReadingPipe(0, address_odbiorczy);
  ...
  if (radio.available()) 
  {
    ...
    if (recievedMessage[0] == '0')
    {
      ...
      // część nadawcza
        ...
        long light_ = analogRead(fotoResPIN);
        double light = 100.0 - (light_*100.0/1024.0);
        ...
        if (sensors.getDeviceCount() > 0)
        {
          ...
          if (sensors.getTempC(tempDeviceAddress) < 0 || sensors.getTempC(tempDeviceAddress) > 80)
            tempC = 99;
          else
        ...
        commandString += "_";
        commandString += digitalRead(buttonIn);
        char command[32];
